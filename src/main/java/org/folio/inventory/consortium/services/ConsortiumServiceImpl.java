...
public class ConsortiumServiceImpl implements ConsortiumService {
  ...
  public Future<Optional<ConsortiumConfiguration>> getConsortiumConfiguration(Context context) {
    LOGGER.info("getConsortiumConfiguration:: Retrieving consortium configuration for tenantId: {}",
      context.getTenantId());
    ...
    CompletableFuture<SharingInstance> completableFuture = createOkapiHttpClient(context, httpClient)
      .thenCompose(client ->
        client.post(context.getOkapiLocation() + String.format(SHARE_INSTANCE_ENDPOINT, consortiumId),
            Json.encode(sharingInstance), headers)
          .thenCompose(httpResponse -> {
            if (httpResponse.statusCode() == HttpStatus.SC_CREATED && !SharingStatus.ERROR.toString()
              .equals(httpResponse.getJson().getString("status"))) {
              ...
              LOGGER.debug("shareInstance:: Successfully sharedInstance with id: {}, sharedInstance: {}",
                response.getInstanceIdentifier(), httpResponse.body());
              return CompletableFuture.completedFuture(response);
            } else {
              String message = String.format(SHARING_INSTANCE_ERROR, sharingInstance.getSourceTenantId(),
                sharingInstance.getTargetTenantId(),
                sharingInstance.getInstanceIdentifier(), httpResponse.statusCode(), httpResponse.body());
