...
public class CommonMatchEventHandler implements EventHandler {
  ...
  private static final String MULTIPLE_RECORDS_FOUND_MSG =
    "Found multiple records corresponding to match profile criteria, incomingRecordId: '%s', jobExecutionId: '%s'";
  ...
  public CompletableFuture<DataImportEventPayload> handle(DataImportEventPayload eventPayload) {
    try {
      ...
      if (handlerOptional.isPresent()) {
        ...
        String msg =
          format("Event payload with current node '%s' is not supported, incomingRecordId: '%s', jobExecutionId: '%s'",
            eventPayload.getCurrentNode().getContentType(), extractIncomingRecordId(eventPayload),
  ...
  private CompletableFuture<DataImportEventPayload> invokeNextHandlerIfNeeded(DataImportEventPayload eventPayload) {
    ...
    if (nextProfileOptional.isPresent() && nextProfileOptional.get().getContentType() == MATCH_PROFILE) {
      ...
      if (optionalEventHandler.isPresent()) {
        LOG.debug(
          "invokeNextHandlerIfNeeded:: Invoking '{}' event handler, incomingRecordId: '{}', jobExecutionId: '{}'",
          () -> optionalEventHandler.get().getClass().getSimpleName(), () -> extractIncomingRecordId(eventPayload),
          eventPayload::getJobExecutionId);
      ...
      String msg = format("No suitable handler found to handle multiple match result, eventType: '%s'",
        eventPayload.getEventType());
      return CompletableFuture.failedFuture(new EventProcessingException(msg));
    }

    String msg =
      format(MULTIPLE_RECORDS_FOUND_MSG, extractIncomingRecordId(eventPayload), eventPayload.getJobExecutionId());
