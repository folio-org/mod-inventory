...
public class CreateMarcHoldingsEventHandler implements EventHandler {
  ...
  private static final String CONTEXT_EMPTY_ERROR_MESSAGE =
    "Can`t create Holding entity: context is empty or doesn't exist";
  private static final String ACTION_HAS_NO_MAPPING_MSG =
  ...
  public boolean isEligible(DataImportEventPayload dataImportEventPayload) {
    if (dataImportEventPayload.getCurrentNode() != null
        && dataImportEventPayload.getContext().get(MARC_HOLDINGS.value()) != null
  ...
  private void defaultMapRecordToHoldings(DataImportEventPayload dataImportEventPayload,
    ...
    try {
      ...
      dataImportEventPayload.getContext()
        .put(HOLDINGS.value(), Json.encode(new JsonObject().put(HOLDINGS_PATH, JsonObject.mapFrom(holdings))));
    } catch (Exception e) {
  ...
  private Future<String> findInstanceIdByHrid(DataImportEventPayload dataImportEventPayload, JsonObject holdingAsJson,
                                              Context context) {
              ...
              .compose(consortiumConfigurationOptional -> {
                if (consortiumConfigurationOptional.isPresent()) {
                  ...
                  return holdingsCollectionService.findInstanceIdByHrid(consortiumInstanceCollection, instanceHrid)
                    .compose(centralInstanceId -> consortiumService.createShadowInstance(context, centralInstanceId,
  ...
  private Future<HoldingsRecord> addHoldings(HoldingsRecord holdings,
    ...
    holdingsRecordCollection.add(holdings,
      success -> promise.complete(success.result()),
      failure -> {
        ...
        if (isNotBlank(failure.reason()) && failure.reason().contains(UNIQUE_ID_ERROR_MESSAGE)) {
          ...
          LOGGER.error(
            format("Error posting Holdings cause %s, status code %s", failure.reason(), failure.statusCode()));
