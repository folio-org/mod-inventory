...
public class MarcBibModifiedPostProcessingEventHandler implements EventHandler {
  ...
  public CompletableFuture<DataImportEventPayload> handle(DataImportEventPayload dataImportEventPayload) {
    ...
    try {
      ...
      Context targetInstanceContext =
        EventHandlingUtil.constructContext(getTenant(dataImportEventPayload), dataImportEventPayload.getToken(),
          dataImportEventPayload.getOkapiUrl(),
          payloadContext.get(DataImportHeaders.USER_ID), payloadContext.get(XOkapiHeaders.REQUEST_ID.toLowerCase()));
      ...
      mappingMetadataCache.get(dataImportEventPayload.getJobExecutionId(), localTenantContext)
        ...
        .compose(precedingSucceedingTitles -> precedingSucceedingTitlesHelper.deletePrecedingSucceedingTitles(
          precedingSucceedingTitles, targetInstanceContext))
        .compose(
          ...
          if (updateAr.succeeded()) {
            ...
            if (updateAr.cause() instanceof OptimisticLockingException) {
              ...
              LOGGER.error("Error updating inventory instance by id: '{}' by jobExecutionId: '{}'", instanceId,
                dataImportEventPayload.getJobExecutionId(), updateAr.cause());
  ...
  public boolean isEligible(DataImportEventPayload dataImportEventPayload) {
    if (dataImportEventPayload.getCurrentNode() != null && MAPPING_PROFILE == dataImportEventPayload.getCurrentNode()
      .getContentType()) {
      MappingProfile mappingProfile =
        JsonObject.mapFrom(dataImportEventPayload.getCurrentNode().getContent()).mapTo(MappingProfile.class);
      return
        DI_SRS_MARC_BIB_RECORD_MODIFIED_READY_FOR_POST_PROCESSING.value().equals(dataImportEventPayload.getEventType())
  ...
  private void processOLError(DataImportEventPayload dataImportEventPayload,
    ...
    if (currentRetryNumber < MAX_RETRIES_COUNT) {
      ...
      LOGGER.warn("Error updating Instance - {}. Retry MarcBibModifiedPostProcessingEventHandler handler...",
        updateAr.cause().getMessage());
      ...
      dataImportEventPayload.getContext().remove(CURRENT_RETRY_NUMBER);
      String errMessage =
        format("Current retry number %s exceeded given number %s for the Instance update", MAX_RETRIES_COUNT,
          currentRetryNumber);
