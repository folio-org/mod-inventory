...
public abstract class AbstractMarcMatchEventHandler implements EventHandler {
  ...
  private static final String PAYLOAD_HAS_NO_DATA_MESSAGE =
  ...
  public CompletableFuture<DataImportEventPayload> handle(DataImportEventPayload payload) {
    try {
      ...
      payload.setAdditionalProperty(DataImportHeaders.USER_ID, context.get(DataImportHeaders.USER_ID));
      ...
      if (!isValidMatchDetail(matchDetail)) {
        LOG.warn("handle:: Match detail is invalid, jobExecutionId: '{}', match detail: '{}'",
          payload.getJobExecutionId(), matchDetail);
      ...
      return createRecordsMatchingContext(payload)
        ...
        .onFailure(e -> LOG.warn("handle:: Failed to process event for MARC record matching, jobExecutionId: '{}'",
  ...
  public boolean isEligible(DataImportEventPayload dataImportEventPayload) {
    if (dataImportEventPayload.getCurrentNode() != null
      ...
      var matchProfile =
        JsonObject.mapFrom(dataImportEventPayload.getCurrentNode().getContent()).mapTo(MatchProfile.class);
  ...
  private Future<RecordsMatchingContext> createRecordsMatchingContext(DataImportEventPayload payload) {
    String userId = payload.getContext().get(DataImportHeaders.USER_ID);
    String requestId = payload.getContext().get(XOkapiHeaders.REQUEST_ID.toLowerCase());
    ...
    return consortiumService.getConsortiumConfiguration(context).map(consortiumConfigurationOptional -> {
      consortiumConfigurationOptional.ifPresent(consortiumConfiguration -> {
        recordsMatchingContext.setCentralTenantId(consortiumConfiguration.centralTenantId());
        recordsMatchingContext.setCentralTenantRecordsClient(
          getSourceStorageRecordsClient(payload, consortiumConfiguration.centralTenantId()));
      });
  ...
  private SourceStorageRecordsClient getSourceStorageRecordsClient(DataImportEventPayload payload, String tenantId) {
    var folioHeaders = FolioHeaders.builder()
      ...
      .userId(payload.getContext().get(DataImportHeaders.USER_ID))
      .token(payload.getToken())
      .requestId(payload.getContext().get(XOkapiHeaders.REQUEST_ID.toLowerCase()))
      .tenant(tenantId);
  ...
  private Future<Optional<Record>> matchRecords(RecordMatchingDto recordMatchingDto,
    ...
    return Future.all(localMatchingFuture, centralMatchingFuture).compose(v -> {
      ...
      if (localMatchingRes.getTotalRecords() == 1 && centralMatchingRes.getIdentifiers().isEmpty()) {
        return getRecordById(localMatchingRes.getIdentifiers().getFirst(), localTenantStorageRecordsClient,
          payload).map(Optional::of);
        ...
        return getRecordById(centralMatchingRes.getIdentifiers().getFirst(),
          recordsMatchingContext.getCentralTenantRecordsClient(), payload).map(Optional::of);
  ...
  private Future<Record> getRecordById(RecordIdentifiersDto recordIdentifiersDto,
    ...
    return sourceStorageRecordsClient.getSourceStorageRecordsById(recordId)
      .compose(response -> {
        ...
        String msg = format(
          "Failed to retrieve record by id: '%s', responseStatus: '%s', body: '%s', jobExecutionId: '%s', tenant: '%s'",
  ...
  private Future<RecordsIdentifiersCollection> getMatchedRecordsIdentifiersOnCentralTenantIfNeeded(
    RecordMatchingDto recordMatchingDto,
    RecordsMatchingContext recordsMatchingContext,
    DataImportEventPayload payload) {
    if (isMatchingOnCentralTenantRequired() && nonNull(recordsMatchingContext.getCentralTenantId())
      ...
      return getAllMatchedRecordsIdentifiers(recordMatchingDto, payload,
        recordsMatchingContext.getCentralTenantRecordsClient());
    }
   ...
   */
  private Future<DataImportEventPayload> processSucceededResult(Optional<Record> recordOptional,
                                                                DataImportEventPayload payload) {
  ...
  private boolean canNextProfileProcessMultiMatchResult(DataImportEventPayload eventPayload) {
    ...
    if (isNotEmpty(childProfiles) && ReactToType.MATCH.equals(childProfiles.getFirst().getReactTo())
        && MATCH_PROFILE.equals(childProfiles.getFirst().getContentType())) {
      MatchProfile nextMatchProfile =
        JsonObject.mapFrom(childProfiles.getFirst().getContent()).mapTo(MatchProfile.class);
