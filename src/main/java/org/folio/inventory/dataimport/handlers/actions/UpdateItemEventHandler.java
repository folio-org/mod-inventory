...
public class UpdateItemEventHandler implements EventHandler {
  ...
  private static final Set<String> PROTECTED_STATUSES_FROM_UPDATE = new HashSet<>(
    Arrays.asList("Aged to lost", "Awaiting delivery", "Awaiting pickup", "Checked out", "Claimed returned",
      "Declared lost", "Paged", "Recently returned"));
  ...
  private final List<String> requiredFields =
    Arrays.asList("status.name", "materialType.id", "permanentLoanType.id", "holdingsRecordId");
  ...
  public CompletableFuture<DataImportEventPayload> handle(DataImportEventPayload dataImportEventPayload) {
    ...
    try {
      ...
      mappingMetadataCache.get(jobExecutionId, context)
        ...
        .onSuccess(mappingMetadataDto -> {
          if (dataImportEventPayload.getContext().containsKey(MULTIPLE_HOLDINGS_FIELD)) {
            dataImportEventPayload.getContext()
              .put(TEMPORARY_MULTIPLE_HOLDINGS_FIELD, dataImportEventPayload.getContext().get(MULTIPLE_HOLDINGS_FIELD));
          }
          ...
          for (int i = 0; i < itemsJsonArray.size(); i++) {
            ...
            if (!validationErrors.isEmpty()) {
              String msg =
                format("Mapped Item is invalid: %s, by jobExecutionId: '%s' and recordId: '%s' and chunkId: '%s' ",
                  validationErrors,
                  jobExecutionId, recordId, chunkId);
              LOGGER.warn("handle:: {}", msg);
              dataImportEventPayload.getContext().put(ERRORS, Json.encode(validationErrors));
              errors.add(new PartialError(
                mappedItemAsJson.getString(ID_PATH_FIELD) != null ? mappedItemAsJson.getString(ID_PATH_FIELD) : BLANK,
                msg));
              ...
              isProtectedStatusChanged.set(
                isProtectedStatusChanged(oldItemStatuses.get(mappedItemAsJson.getString(ID_PATH_FIELD)),
                  newItemStatus));
              if (isProtectedStatusChanged.get()) {
                mappedItemAsJson.getJsonObject(STATUS_KEY)
                  .put("name", oldItemStatuses.get(mappedItemAsJson.getString(ID_PATH_FIELD)));
              }
              ...
              verifyItemBarcodeUniqueness(itemToUpdate, itemCollection, updatePromise, errors)
                ...
                .onSuccess(updatedItem -> {
                  if (isProtectedStatusChanged.get()) {
                    String msg =
                      format(STATUS_UPDATE_ERROR_MSG, oldItemStatuses.get(updatedItem.getId()), newItemStatus);
                    LOGGER.warn("handle:: {}", msg);
        ...
        .onFailure(e -> {
          LOGGER.warn(
            "handle:: Failed to update inventory Item by jobExecutionId: '{}' and recordId: '{}' and chunkId: '{}' ",
            jobExecutionId,
            recordId, chunkId, e);
  ...
  private Future<DataImportEventPayload> addHoldingToPayloadIfNeeded(DataImportEventPayload dataImportEventPayload,
                                                                     Context context, Item updatedItem) {
    ...
    JsonArray holdings = new JsonArray(
      dataImportEventPayload.getContext().get(HOLDINGS.value()) != null ? dataImportEventPayload.getContext()
        .get(HOLDINGS.value()) : BLANK_JSON_ARRAY);
    if (holdings.stream()
      .noneMatch(holding -> Strings.CS.equals(((JsonObject) holding).getString("id"), updatedItem.getHoldingId()))) {
      holdingsRecordCollection.findById(updatedItem.getHoldingId(),
        success -> {
          LOGGER.info("addHoldingToPayloadIfNeeded:: Successfully retrieved Holdings for the hotlink by id: {}",
            updatedItem.getHoldingId());
          holdings.add(success.result());
          dataImportEventPayload.getContext().put(HOLDINGS.value(), holdings.encode());
          promise.complete(dataImportEventPayload);
        },
        failure -> {
          LOGGER.warn(
            "addHoldingToPayloadIfNeeded:: Error retrieving Holdings for the hotlink by id {} cause {}, status code {}",
            updatedItem.getHoldingId(), failure.reason(), failure.statusCode());
          promise.complete(dataImportEventPayload);
        });
    } else {
      LOGGER.debug("addHoldingToPayloadIfNeeded:: Holdings already exists in payload with for the hotlink with id {}",
        updatedItem.getHoldingId());
      promise.complete(dataImportEventPayload);
    }
  ...
  private void preparePayloadForMappingManager(DataImportEventPayload dataImportEventPayload) {
    ...
    dataImportEventPayload.getContext()
      .put(CURRENT_NODE_PROPERTY, Json.encode(dataImportEventPayload.getCurrentNode()));
  ...
  private Future<Void> verifyItemBarcodeUniqueness(Item item, ItemCollection itemCollection,
    ...
    try {
      itemCollection.findByCql(CqlHelper.barcodeIs(item.getBarcode()) + " AND id <> " + item.id,
        PagingParameters.defaults(),
        findResult -> {
          if (findResult.result().records().isEmpty()) {
            promise.complete();
          } else {
            LOGGER.warn("verifyItemBarcodeUniqueness:: Barcode must be unique, {} is already assigned to another item",
              item.getBarcode());
            errors.add(new PartialError(item.getId() != null ? item.getId() : BLANK,
              format("Barcode must be unique, %s is already assigned to another item", item.getBarcode())));
            updatePromise.complete();
            promise.fail(format("Barcode must be unique, %s is already assigned to another item", item.getBarcode()));
          }
        },
        failure -> {
          errors.add(new PartialError(item.getId() != null ? item.getId() : BLANK, failure.reason()));
          updatePromise.complete();
      ...
      errors.add(new PartialError(item.getId() != null ? item.getId() : BLANK,
        format("Failed to find items by barcode '%s'", item.getBarcode())));
      updatePromise.complete();
      promise.fail(msg);
    }
    return promise.future();
    ...
    itemCollection.update(item, success -> promise.complete(item),
      failure -> {
        if (failure.statusCode() == HttpStatus.SC_CONFLICT) {
          ...
          errors.add(new PartialError(item.getId() != null ? item.getId() : BLANK, failure.reason()));
          LOGGER.warn("updateItemAndRetryIfOLExists:: updating Item - {}, status code {}", failure.reason(),
            failure.statusCode());
        }
        updatePromise.complete();
        promise.fail(failure.reason());
    ...
    if (currentRetryNumber < MAX_RETRIES_COUNT) {
      ...
      getActualItemsList(expiredItems, itemCollection)
        .onSuccess(
          actualItemsList -> prepareDataAndReInvokeCurrentHandler(dataImportEventPayload, future, actualItemsList,
            errors, olAccumulativeResults))
        .onFailure(e -> {
          String errMessage =
            format("Cannot get actual Items.Expired Items: '%s' for jobExecutionId '%s'. Error: %s ", expiredItems,
              dataImportEventPayload.getJobExecutionId(), e.getCause());
      ...
      String errMessage =
        format("Current retry number %s exceeded or equal given number %s for the Item update for jobExecutionId '%s' ",
          MAX_RETRIES_COUNT, currentRetryNumber, dataImportEventPayload.getJobExecutionId());
  ...
  private void prepareDataAndReInvokeCurrentHandler(DataImportEventPayload dataImportEventPayload,
                                                    CompletableFuture<DataImportEventPayload> future,
                                                    List<Item> actualItems, List<PartialError> errors,
                                                    OlItemAccumulativeResults olAccumulativeResults) {
    ...
    dataImportEventPayload.getEventsChain()
      .remove(dataImportEventPayload.getContext().get(CURRENT_EVENT_TYPE_PROPERTY));
    try {
      dataImportEventPayload.setCurrentNode(ObjectMapperTool.getMapper()
        .readValue(dataImportEventPayload.getContext().get(CURRENT_NODE_PROPERTY), ProfileSnapshotWrapper.class));
    ...
    dataImportEventPayload.getContext()
      .put(MULTIPLE_HOLDINGS_FIELD, dataImportEventPayload.getContext().get(TEMPORARY_MULTIPLE_HOLDINGS_FIELD));
    dataImportEventPayload.getContext().remove(TEMPORARY_MULTIPLE_HOLDINGS_FIELD);
    olAccumulativeResults.getResultedErrorItems().addAll(errors);
  ...
  private static JsonArray convertItemListAsJsonArray(List<Item> actualItems) {
    ...
    for (Item currentItem : actualItems) {
      resultedItems.add(
        new JsonObject().put(ITEM_PATH_FIELD, new JsonObject(ItemUtil.mapToMappingResultRepresentation(currentItem))));
    }
  ...
  private Future<List<Item>> getActualItemsList(List<Item> items, ItemCollection itemsCollection) {
    ...
    try {
      itemsCollection.findByCql(format("id==(%s)", getQueryParamForMultipleItems(items)), PagingParameters.defaults(),
        findResults -> {
          List<Item> actualItems = findResults.result().records();
  ...
  private void fillPayloadAndClearLists(DataImportEventPayload dataImportEventPayload, String errorsAsStringJson,
                                        CompletableFuture<DataImportEventPayload> future,
                                        OlItemAccumulativeResults olAccumulativeResults) {
    dataImportEventPayload.getContext()
      .put(ActionProfile.FolioRecord.ITEM.value(), Json.encode(olAccumulativeResults.getResultedSuccessItems()));
  ...
  private static OlItemAccumulativeResults constructOlAccumulativeResults(
    DataImportEventPayload dataImportEventPayload) {
    OlItemAccumulativeResults olAccumulativeResults;
    JsonObject olAccumulativeResultsAsJson =
      new JsonObject(dataImportEventPayload.getContext().get(OL_ACCUMULATIVE_RESULTS));
