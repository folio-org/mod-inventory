...
public class OrderHelperServiceImpl implements OrderHelperService {
  ...
  public Future<Void> fillPayloadForOrderPostProcessingIfNeeded(DataImportEventPayload eventPayload,
                                                                DataImportEventTypes targetEventType, Context context) {
  ...
  private CompletableFuture<Void> checkIfOrderLogicExistsAndFillPayloadIfNeeded(DataImportEventPayload eventPayload,
    ...
    if (!actionProfiles.isEmpty() && checkIfOrderActionProfileExists(actionProfiles)
        && checkIfCurrentProfileIsTheLastOne(eventPayload, actionProfiles)) {
  ...
  private static boolean checkIfCurrentProfileIsTheLastOne(DataImportEventPayload eventPayload,
                                                           List<ProfileSnapshotWrapper> actionProfiles) {
    String currentMappingProfileId = eventPayload.getCurrentNode().getProfileId();
    ProfileSnapshotWrapper lastActionProfile = actionProfiles.getLast();
    ...
    if (childSnapshotWrappers != null && childSnapshotWrappers.getFirst() != null && Objects.equals(
      childSnapshotWrappers.getFirst().getContentType().value(), "MAPPING_PROFILE")) {
      mappingProfileId = childSnapshotWrappers.getFirst().getProfileId();
    }
  ...
  private static boolean checkIfOrderActionProfileExists(List<ProfileSnapshotWrapper> actionProfiles) {
    for (ProfileSnapshotWrapper actionProfile : actionProfiles) {
      LinkedHashMap<String, String> content =
        new ObjectMapper().convertValue(actionProfile.getContent(), LinkedHashMap.class);
