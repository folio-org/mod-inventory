...
public abstract class AbstractInstances {
  ...
  private static final String LINKING_SHARED_AND_LOCAL_INSTANCE_ERROR =
    "One instance is local and one is shared. To be linked, both instances must be local or shared.";
  protected final Storage storage;
  protected final HttpClient client;
  protected final InventoryConfiguration config;
  private final ConsortiumService consortiumService;

  protected AbstractInstances(final Storage storage, final HttpClient client, ConsortiumService consortiumService) {
   ...
   * @param context        The web context for the request.
   */
  protected CompletableFuture<List<Response>> updateInstanceRelationships(Instance instance,
    ...
    CollectionResourceRepository instanceRelationshipsRepository =
      new CollectionResourceRepository(instanceRelationshipsClient);
  ...
  protected JsonObject toRepresentation(InstancesResponse instancesResponse) {
    ...
    MultipleRecords<Instance> wrappedInstances = instancesResponse.getSuccess().result();
    List<Instance> instances = wrappedInstances.records();

    instances.forEach(instance -> {
      List<InstanceRelationshipToParent> parentInstances =
        instancesResponse.getParentInstanceMap().get(instance.getId());
      ...
      List<PrecedingSucceedingTitle> succeedingTitles =
        instancesResponse.getSucceedingTitlesMap().get(instance.getId());
      results.add(instance
        .setParentInstances(parentInstances)
        .setChildInstances(childInstances)
        .setPrecedingTitles(precedingTitles)
        .setSucceedingTitles(succeedingTitles).getJsonForResponse());
    });
    ...
    return consortiumService.getConsortiumConfiguration(context).toCompletionStage().toCompletableFuture()
      .thenCompose(consortiumConfigurationOptional -> {
        ...
        if (consortiumConfigurationOptional.isPresent()) {
          List<InstanceRelationship> newRelationships =
            updatingRelationships.keySet().stream().filter(key -> !existingRelationships.containsKey(key))
    ...
    return InstanceUtil.findInstanceById(instanceId, storage.getInstanceCollection(context))
      .recover(throwable -> {
        if (throwable instanceof NotFoundException) {
          log.warn("confirmInstanceExistAtLocalMember:: Trying to link shared and local instance, instanceId: {}",
            instanceId);
  ...
  private static String getRelatedInstanceId(Instance instance, InstanceRelationship relationship) {
    return !relationship.superInstanceId().equals(instance.getId()) ? relationship.superInstanceId()
  ...
  private <T> List<CompletableFuture<Response>> createOrEdit(CollectionResourceRepository resourceClient,
                                                             Map<String, T> existingObjects,
                                                             Map<String, T> updatingObjects) {
  ...
  private CollectionResourceClient getCollectionResourceRepository(RoutingContext routingContext,
