...
public final class MarkAsUnavailableValidators {
  private static final Set<ItemStatusName> ALLOWED_STATUS_TO_MARK_UNAVAILABLE = of(
  ...
  private MarkAsUnavailableValidators() {}

  public static CompletableFuture<Item> itemHasAllowedStatusToMarkAsUnavailable(Item item) {
   if (ALLOWED_STATUS_TO_MARK_UNAVAILABLE.contains(item.getStatus().getName())) {
      return CompletableFuture.completedFuture(item);
    }

    return failedFuture(new UnprocessableEntityException(
      new ValidationError("Item is not allowed to be marked as Unavailable",
