...
class BoundWithTest extends ApiTests {
  ...
  void boundWithFlagsArePresentOnInstancesAndItemsAsExpected()
    ...
    IndividualResource instance2 = instancesStorageClient.create(InstanceFixture.girlOnTheTrain(UUID.randomUUID()));
    ...
    IndividualResource instance3 = instancesStorageClient.create(InstanceFixture.leviathanWakes(UUID.randomUUID()));
    ...
    JsonObject boundWithPart =
      new BoundWithPartRequestBuilder(item1a.getJson().getString("id"), holdings2a.getJson().getString("id")).create();
    ...
    for (JsonObject instance : instancesClient.getAll()) {
      if (instance.getString("id").equals(instance1.getJson().getString("id"))) {
        assertThat("Instance 1 is bound-with: " + instance.encodePrettily(), instance.getBoolean("isBoundWith"),
          is(true));
      } else if (instance.getString("id").equals(instance2.getJson().getString("id"))) {
        assertThat("Instance 2 is bound-with: " + instance.encodePrettily(), instance.getBoolean("isBoundWith"),
          is(true));
      } else if (instance.getString("id").equals(instance3.getJson().getString("id"))) {
        assertThat("Instance 3 is NOT bound-with: " + instance.encodePrettily(), instance.getBoolean("isBoundWith"),
          is(false));
      } else {
    ...
    assertThat("instance 3 fetched by ID is NOT bound-with", instance3byId.getJson().getBoolean("isBoundWith"),
      is(false));
  ...
  @Test
  void willSetBoundWithFlagsOnManyItemsContainingTheSameTitle()
    throws InterruptedException, MalformedURLException, TimeoutException, ExecutionException {
    IndividualResource instance1 = instancesStorageClient.create(InstanceFixture.smallAngryPlanet(UUID.randomUUID()));
    IndividualResource holdings1a = holdingsStorageClient.create(new HoldingRequestBuilder()
      .forInstance(instance1.getId()).permanentlyInMainLibrary().withCallNumber("HOLDINGS 1A"));
    ...
    for (int i = 0; i < 200; i++) {
      ...
      boundWithPartsStorageClient.create(
        new BoundWithPartRequestBuilder(item.getJson().getString("id"), holdings1a.getJson().getString("id")).create());
    }
    for (int i = 0; i < 100; i++) {
      IndividualResource item = itemsClient.create(new ItemRequestBuilder()
  ...
  @Test
  void boundWithTitlesArePresentInBoundWithItem() throws InterruptedException, TimeoutException, ExecutionException {
    IndividualResource instance1 = instancesStorageClient.create(InstanceFixture.smallAngryPlanet(UUID.randomUUID()));
    IndividualResource holdings1a = holdingsStorageClient.create(new HoldingRequestBuilder()
      .forInstance(instance1.getId()).permanentlyInMainLibrary().withCallNumber("HOLDINGS 1A"));
    IndividualResource item1a = itemsClient.create(new ItemRequestBuilder()
    ...
    boundWithPartsStorageClient.create(
      new BoundWithPartRequestBuilder(item1a.getJson().getString("id"), holdings1a.getJson().getString("id")).create());
    boundWithPartsStorageClient.create(
      new BoundWithPartRequestBuilder(item1a.getJson().getString("id"), holdings2a.getJson().getString("id")).create());
    boundWithPartsStorageClient.create(
  ...
  void canRetrieveBoundWithItemByHoldingsRecordId() throws InterruptedException, TimeoutException, ExecutionException {
    ...
    assertThat("Two items are found for holdings record: " + itemsResponse1.getJson().encodePrettily(),
      itemsResponse1.getJson().getInteger("totalRecords"), is(2));
    ...
    assertThat("The bound-with item returned is indeed flagged as a bound-with", item2.getBoolean("isBoundWith"),
      is(true));
  ...
  @Test
  void canRetrieveManySingleTitleItemsThroughItemsByHoldingsId()
    throws InterruptedException, TimeoutException, ExecutionException {
    IndividualResource instance1 =
      instancesStorageClient.create(InstanceFixture.smallAngryPlanet(UUID.randomUUID()).put("title", "Instance 1"));
  ...
  void canRetrieveManyBoundWithsAndRegularItemsThroughItemsByHoldingsId()
    ...
    assertThat("total records is 400 with 'onlyBoundWiths'",
      itemsByHoldingsIdResponseOnlyBoundWiths.getJson().getInteger("totalRecords"), is(400));
  }

  @Test
  void canFetchBoundWithItemWithManyParts() throws InterruptedException, TimeoutException, ExecutionException {
    IndividualResource instance1 =
      instancesStorageClient.create(InstanceFixture.smallAngryPlanet(UUID.randomUUID()).put("title", "Instance 1"));
    IndividualResource holdings1 = holdingsStorageClient.create(new HoldingRequestBuilder()
      .forInstance(instance1.getId()).permanentlyInMainLibrary().withCallNumber("HOLDINGS 1"));
    ...
    boundWithPartsStorageClient.create(
      new BoundWithPartRequestBuilder(item.getJson().getString("id"), holdings1.getJson().getString("id")).create());

    for (int i = 2; i <= 200; i++) {
      IndividualResource instance = instancesStorageClient.create(
        InstanceFixture.smallAngryPlanet(UUID.randomUUID()).put("title", "Instance " + i));
